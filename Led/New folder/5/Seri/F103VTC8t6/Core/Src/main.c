...
void displayTimeOn7Segment(uint32_t time) {
    ...
    sprintf(displayBuffer, "%05lu", (unsigned long) time);
		for(int i = 0; i < 5; i++) {
...
// Ham PWM
void set_compare_value(uint32_t PWM)
{
...
int main(void)
{
  ...
  while (1)
  {
// 	Ham thay doi gia tri cua PWM.	
//set_compare_value(ms);
		
// Ham thay doi chu ky timer
