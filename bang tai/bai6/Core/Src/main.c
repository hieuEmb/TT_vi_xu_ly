...
//Ham gui len uart gia tri chui.
void UART_Send_String(const char *str) {
    HAL_UART_Transmit(&huart1, (uint8_t *)str, strlen(str), HAL_MAX_DELAY); // G?i chu?i qua UART
}
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
		...
		if ((P1==0)&& (AUTO==0))// nut xanh
		{
			HAL_GPIO_WritePin(K_GPIO_Port,K_Pin, GPIO_PIN_RESET);
			HAL_GPIO_WritePin(D1_GPIO_Port,D1_Pin, GPIO_PIN_RESET);
			if (S1==0&&(AUTO==0))// Neu co vat thi tiep tuc quay.
				{
					HAL_GPIO_WritePin(K_GPIO_Port,K_Pin, GPIO_PIN_RESET);
					UART_Send_String("Runing");
				}
			if (S1==1&&(AUTO==0))
				{
					HAL_Delay(4000);
